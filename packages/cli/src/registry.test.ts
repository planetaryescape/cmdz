...
test('autostarts configured commands and isolates optional panes and input', async () => {
  ...
  try {
    ...
    const secondRow = rows.findIndex((line) => line.slice(0, 27).includes('Second'))
    const firstRow = rows.findIndex((line) => line.slice(0, 27).includes('First'))
