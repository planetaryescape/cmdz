...
test('rejects malformed, empty, duplicate, and invalid directory definitions', async () => {
  ...
  for (const input of inputs) {
    expect(Exit.isFailure(await Effect.runPromiseExit(parseConfig(process.cwd())(input)))).toBe(
      true,
    )
  }
})
...
test('loads a TypeScript default export relative to its config file', async () => {
  ...
  try {
    ...
    expect(Exit.isFailure(await Effect.runPromiseExit(loadConfig(join(root, 'missing.ts'))))).toBe(
      true,
    )
  } finally {
    await rm(root, { recursive: true })
