...
test("autostarts configured commands and isolates optional panes and input", async () => {
  ...
  const running = Effect.runPromiseExit(processWorkspace(ui.renderer, [
    { name: "First", title: "First", command: 'printf "FIRST_READY\\n"; read value; printf "FIRST:%s\\n" "$value"; read final', cwd: process.cwd(), env: {}, autostart: true },
    { name: "Second", title: "Second", command: 'printf "SECOND_READY\\n"; read value; printf "SECOND:%s\\n" "$value"; read final', cwd: process.cwd(), env: {}, autostart: false },
