...
type Action =
  ...
  | { readonly type: "exited"; readonly pane: ProcessPane; readonly run: number; readonly code: number }
...
export const processWorkspace = Effect.fn("process.workspace")(function* (
  ...
  while (true) {
    ...
    if (action.type === "exited" || action.type === "failed") {
      ...
      pane.status = action.type === "failed" ? "failed" : action.code === 0 ? "succeeded" : `failed (${action.code})`;
    ...
    pane.fiber = yield* runPty(
      ["/bin/sh", "-c", pane.definition.command],
      {
        ...
        attach: (value) => {
          pane.pty = value;
          if (value) { pane.status = "running"; drawStatus(); }
        },
      },
